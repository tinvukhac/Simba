...
object Hello {
  ...
  case class Trajectory(trajId: Long, seqId: Long, lon: Double, lat:Double, time: Timestamp)
 ...
 def main(args: Array[String]): Unit = {
   val simba = SimbaSession
    ...
    val df4 = df3.select($"trajId", $"seqId", $"lon", $"lat", unix_timestamp($"time", "yyyy-MM-dd HH:mm:ss").cast("timestamp").alias("timestamp"))
    val ds = df4.map(row => Trajectory(row.getString(0).toLong, row.getString(1).toLong, 
        row.getString(2).toDouble, row.getString(3).toDouble, row.getTimestamp(4)))
    ds.printSchema()
    ds.show()
//    ds.limit(1).collect().foreach(trj => println(trj.time))
